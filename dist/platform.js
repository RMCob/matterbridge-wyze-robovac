...
export class RoboVacMatterbridgeDynamicPlatform extends MatterbridgeDynamicPlatform {
    discoverDevicesFinished = false;
    getCurrentStatusFinished = false;
    getRoomsFinished = false;
    startSweepFinished = false;
    startCleanFinished = false;
    stopSweepFinished = false;
    pauseSweepFinished = false;
    continueSweepFinished = false;
    setFloorFinished = false;
    nickname = 'RoboVac';
    macAddr = '';
    currentStatus = '';
    oldStatus = '';
    ...
    isDocked = true;
    errorCode = 'None';
    newRoomsSelected = false;
    pokeStatusLoop = false;
    statusRefreshInterval = 0;
    oldRefreshInterval = 0;
    roboticVacuum;
    ...
    constructor(matterbridge, log, config) {
        ...
        if (this.verifyMatterbridgeVersion === undefined || typeof this.verifyMatterbridgeVersion !== 'function'
            ...
            throw new Error(`This plugin requires Matterbridge version >= "3.3.3". Please update Matterbridge from ${this.matterbridge.matterbridgeVersion} to the latest version in the frontend.`);
        }
    ...
    async onStart(reason) {
        this.myLogger(`onStart - RoboVac called with reason:', reason ?? 'none'`);
        ...
        */
        this.roboticVacuum = new RoboticVacuumCleaner('Robot Vacuum', this.macAddr, this.config.enableServerRvc === true ? 'server' : undefined, 1, // currentRunMode
        [
        ...
        if (this.config.enableServerRvc === true) {
            this.log.notice('RVC is in server mode 22');
        }
        ...
        /*
            if( this.roboticVacuum ) {
              this.roboticVacuum.createDefaultBasicInformationClusterServer(this.nickname, this.macAddr, 0xfff1, 'Wyze', 0x8000, 'Robot Vacuum Cleaner');
            }
        */
    ...
    async onShutdown(reason) {
        ...
        this.myLogger(`'onShutdown called - RoboVac with reason:', reason ?? 'none'`);
    }
    ...
    async setCurrentFloor(floorName) {
        ...
        await exec(`python3 ${this.config.path2py_stubs}/setVacuumFloor.py ${tmpstr1} ${tmpstr2}`, (error, stdout, stderr) => {
            if (error) {
    ...
    async vacuumStartClean(nickname) {
        ...
        await exec(`python3 ${this.config.path2py_stubs}/vacuumStartClean.py ${this.config.username} ${this.config.password} ${tmpstr}`, (error, stdout, stderr) => {
            if (error) {
                this.log.info(`error: ${error.message}`);
            ...
            if (stderr) {
                ...
                if (stderr.includes("Robovac is docked")) {
                    if (this.roboticVacuum) {
                        ...
                        this.roboticVacuum.setAttribute('RvcOperationalState', 'operationalState', RvcOperationalState.OperationalState.Docked, this.log);
                        this.roboticVacuum.setAttribute('RvcOperationalState', 'operationalError', { errorStateId: RvcOperationalState.ErrorState.NoError }, this.log);
                    }
                }
                this.startCleanFinished = true;
                return;
    ...
    async vacuumPauseSweeping(nickname) {
        ...
        const tmpstr = `${this.config.key_id} ${this.config.api_key} '${nickname}'`;
        await exec(`python3 ${this.config.path2py_stubs}/vacuumPauseSweep.py ${this.config.username} ${this.config.password} ${tmpstr}`, (error, stdout, stderr) => {
            if (error) {
                this.log.info(`error: ${error.message}`);
    ...
    async vacuumStopSweeping(nickname) {
        ...
        const tmpstr = `${this.config.key_id} ${this.config.api_key} '${nickname}'`;
        await exec(`python3 ${this.config.path2py_stubs}/vacuumStopSweep.py ${this.config.username} ${this.config.password} ${tmpstr}`, (error, stdout, stderr) => {
            if (error) {
                this.log.info(`error: ${error.message}`);
    ...
    async vacuumStartSweeping(nickname, rooms2sweep) {
        let roomNames = '';
        if (rooms2sweep.length > 0) {
            for (let i = 0; i < rooms2sweep.length; i++) {
                let areaId = rooms2sweep[i];
                this.myLogger(`before StartSweep, rooms2sweep[${i}] = '${areaId}'`);
                for (let j = 0; j < this.supportedAreas.length; j++) {
                    let area = this.supportedAreas[j];
                    if (area.areaId == areaId) {
                        let name = area.areaInfo.locationInfo.locationName;
                        roomNames += "'" + name + "'" + ' ';
        ...
        await exec(`python3 ${this.config.path2py_stubs}/vacuumStartSweep.py ${this.config.username} ${this.config.password} ${tmpstr}`, (error, stdout, stderr) => {
            if (error) {
                this.log.info(`error: ${error.message}`);
    ...
    async getStatus() {
        ...
        const refreshInterval = await this.getCurrentStatus(this.nickname);
    ...
    async getCurrentStatus(nickname) {
        this.currentStatus = '';
        ...
        const tmpstr = `${this.config.key_id} ${this.config.api_key} '${nickname}'`;
        await exec(`python3 ${this.config.path2py_stubs}/getVacuumStatus.py ${this.config.username} ${this.config.password} ${tmpstr}`, async (error, stdout, stderr) => {
            if (error) {
                this.log.info(`error: ${error.message}`);
            ...
            let line = stdout.slice(0, -1); // Strip off trailing newline ('\n')
            ...
            if (this.roboticVacuum != undefined) {
                ...
                if (this.batteryLevel == 100) {
                    this.roboticVacuum.updateAttribute(PowerSource.Cluster.id, 'batChargeState', PowerSource.BatChargeState.IsAtFullCharge, this.log);
                    ...
                    if (this.isDocked) {
                        this.roboticVacuum.updateAttribute(PowerSource.Cluster.id, 'batChargeState', PowerSource.BatChargeState.IsCharging, this.log);
                        this.batteryWasCharging = true;
                    }
                    else {
                        this.roboticVacuum.updateAttribute(PowerSource.Cluster.id, 'batChargeState', PowerSource.BatChargeState.IsNotCharging, this.log);
                        this.batteryWasCharging = false;
                    }
                }
            }
            ...
            if (this.currentStatus != this.oldStatus) {
                ...
                if (this.currentStatus == 'VacuumMode.IDLE') {
                    if (this.roboticVacuum) {
                        ...
                        await this.roboticVacuum.setAttribute('RvcOperationalState', 'operationalState', RvcOperationalState.OperationalState.Docked, this.log);
                        await this.roboticVacuum.setAttribute('RvcOperationalState', 'operationalError', { errorStateId: RvcOperationalState.ErrorState.NoError }, this.log);
                    }
                    ...
                    if (this.roboticVacuum) {
                        ...
                        await this.roboticVacuum.setAttribute('RvcOperationalState', 'operationalState', RvcOperationalState.OperationalState.Paused, this.log);
                        await this.roboticVacuum.setAttribute('RvcOperationalState', 'operationalError', { errorStateId: RvcOperationalState.ErrorState.NoError }, this.log);
                    }
                    ...
                    if (this.roboticVacuum) {
                        await this.roboticVacuum.setAttribute('RvcRunMode', 'currentMode', 1, this.log); // Idle
                        await this.roboticVacuum.setAttribute('RvcOperationalState', 'operationalState', RvcOperationalState.OperationalState.SeekingCharger, this.log);
                        await this.roboticVacuum.setAttribute('RvcOperationalState', 'operationalError', { errorStateId: RvcOperationalState.ErrorState.NoError }, this.log);
                    }
    ...
    async discoverDevices() {
        ...
        await exec(`python3 ${this.config.path2py_stubs}/getVacuumDeviceList.py ${tmpstr}`, (error, stdout, stderr) => {
            if (error) {
                this.log.info(`error: ${error.message}`);
    ...
    async getRooms(nickname) {
        ...
        await exec(`python3 ${this.config.path2py_stubs}/getVacuumFloors.py ${this.config.username} ${tmpstr}`, (error, stdout, stderr) => {
            if (error) {
                this.log.info(`error: ${error.message}`);
    ...
    floorMapEntry(floorNum, floorName) {
        return {
            mapId: floorNum,
            name: floorName,
        };
    }
    supportedAreaEntry(roomNum, floorNum, roomName) {
        return {
            areaId: roomNum,
            mapId: floorNum,
            areaInfo: {
    ...
    async doSweeping() {
        if (this.currentStatus == 'VacuumMode.IDLE' || this.newRoomsSelected) {
            ...
            if (this.selectedAreas.length > 0) {
                for (let i = 0; i < this.selectedAreas.length; i++) {
                    let areaId = this.selectedAreas[i];
        ...
        if (this.roboticVacuum) {
            await this.roboticVacuum.setAttribute('RvcRunMode', 'currentMode', 2, this.log); // Cleaning
            await this.roboticVacuum.setAttribute('RvcOperationalState', 'operationalState', RvcOperationalState.OperationalState.Running, this.log);
            await this.roboticVacuum.setAttribute('RvcOperationalState', 'operationalError', { errorStateId: RvcOperationalState.ErrorState.NoError }, this.log);
