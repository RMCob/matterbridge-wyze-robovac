...
import { Matterbridge, MatterbridgeEndpoint, MatterbridgeDynamicPlatform, PlatformConfig, bridgedNode, } from 'matterbridge';
...
export class RoboVacMatterbridgeDynamicPlatform extends MatterbridgeDynamicPlatform {
  ...
  constructor(matterbridge: Matterbridge, log: AnsiLogger, config: PlatformConfig) {
    ...
    if (this.verifyMatterbridgeVersion === undefined || typeof this.verifyMatterbridgeVersion !== 'function'
      ...
      throw new Error(
        `This plugin requires Matterbridge version >= "3.3.3". Please update Matterbridge from ${this.matterbridge.matterbridgeVersion} to the latest version in the frontend.`,
      );
    }
...
//   createDefaultBridgedDeviceBasicInformationClusterServer(this.nickname, this.macAddr, 0xfff1, 'Wyze', 'MyToy');
    ...
    this.roboticVacuum.addCommandHandler('goHome', async () => {
	  ...
	  if( this.pokeStatusLoop ) break;   // Something external poked here. Re-evaluate for statusRefreshInterval
      ...
      for (let i = 0; i < rooms2sweep.length; i++) {
        ...
        for (let j = 0; j < this.supportedAreas.length; j++) {
          ...
          if( area.areaId == areaId ) {
            let name : string = area.areaInfo.locationInfo.locationName;
            roomNames += "'" + name + "'" + ' ';
